...
ggplot(mapping = aes(x = Time, y = gini, colour = NUTS2, group = 1, )) + 
  geom_point() 
```
...
```{r}
#pulling out 10 different regions so the plot is more readable
gini_de <- ginigdp %>%
  filter(startsWith(NUTS2, "DE"))
gini_de %>%
  filter(NUTS2 %in% regions) %>%
  ggplot(aes(x = Time, y = gini, colour = NUTS2)) +
  geom_point(mapping = aes()) +
  theme_minimal()
```
