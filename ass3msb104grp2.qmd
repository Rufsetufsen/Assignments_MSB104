...
The theoretical reason for picking these models would be the
...
Empirically, the logarithmic model outperforms the quadratic model on every key metric: coefficient significance, overall model significance, explanatory power and residual variation.
The results show the economic growth has a statistically robust, negative, and diminishing-returns effect on regional inequality, which aligns well with the logarithmic functional form.
In contrast the quadratic model does not provide statistically meaningful evidence of a non-linear relationship, and its low R-squared and non-significant F-test indicate that it fits the data poorly.
For economic growth variable, the logarithmic model is more appropriate to use when showing the connection to regional inequality.
...
Same as the model for economic growth variable, the logarithmic functions provides better.
Although the labour force shows a weak but marginal evidence of a relationship with the
